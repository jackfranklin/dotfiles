...
For runs that install dependencies, `npm install --dangerously-allow-all-scripts` runs if `package.json` exists. Puppeteer performs its normal browser download in its postinstall hook.
