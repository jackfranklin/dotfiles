...
2.  **Identify Changes:** Check for unstaged or staged (uncommitted) changes. If present, review them. If not, review the last commit. **The scope of "review the last commit" vs. "review the whole branch vs. main/base" vs. "review uncommitted changes" is often ambiguous** — especially on a feature branch with several commits already on it, or when the user's request doesn't specify a range. Do not silently default to the narrowest scope (e.g. just the last commit) and present it as the full review. If the intended scope isn't clear from the user's phrasing or context, ask the user which scope they mean before proceeding.
