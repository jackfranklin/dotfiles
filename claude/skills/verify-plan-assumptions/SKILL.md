...
1. **Identify the Plan/Spec**:
   - If the argument is a GitHub issue number, fetch it with `gh issue view <number> --json title,body`.
   - If the argument is a fuzzy description, search with `gh issue list --search "[PLAN] <description>" --state open --json number,title` and pick the best match. If the plan is in the current conversation context, use that directly.
...
2. **Spawn the Verification Subagent**:
   ...
   - **Goal-Oriented Prompt Template**:
     ```
     You are verifying a spec/plan against the live codebase.
...
3. **Integrate Findings**:
   ...
   - Update the GitHub Issue directly to correct these assumptions, or present the findings to the user for guidance.
